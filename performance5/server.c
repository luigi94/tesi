...
#if DILITHIUM_MODE == 2
#define PRIVATE_KEY_NAME_PREFIX "./../DILITHIUM2_AES_keys/srvprvkey%u"
#elif DILITHIUM_MODE == 3
#define PRIVATE_KEY_NAME_PREFIX "./../DILITHIUM3_AES_keys/srvprvkey%u"
#elif DILITHIUM_MODE == 5
#define PRIVATE_KEY_NAME_PREFIX "./../DILITHIUM5_AES_keys/srvprvkey%u"
#endif
...
void make_buffer_and_sign(const char* const restrict to_send_file_name, unsigned char* restrict* const restrict buffer, size_t* const total_len, char* const restrict prvkey_file_name){
  ...
  size_t smlen;
	...
	*total_len = file_len + (size_t)LENGTH_FIELD_LEN + (size_t) TIMESTAMP_LEN;
	smlen = *total_len + (size_t) CRYPTO_BYTES;
	...
	memcpy((void*)(*buffer), (void*)&smlen, LENGTH_FIELD_LEN);
	...
	if((*buffer = (unsigned char*)OPENSSL_realloc(*buffer, smlen)) == NULL){
