...
#if DILITHIUM_MODE == 2
#define PUBLIC_KEY_NAME_PREFIX "./../DILITHIUM2_AES_keys/srvpubkey%u"
#elif DILITHIUM_MODE == 3
#define PUBLIC_KEY_NAME_PREFIX "./../DILITHIUM3_AES_keys/srvpubkey%u"
#elif DILITHIUM_MODE == 5
#define PUBLIC_KEY_NAME_PREFIX "./../DILITHIUM5_AES_keys/srvpubkey%u"
#endif
...
char* results_file_name = "Scenario_5.csv";

size_t time_threshold = 200000UL;
...
int main(int argc, char *argv[]) {
  ...
  uint8_t pk[CRYPTO_PUBLICKEYBYTES];
    ...
    if(mlen != data_size - (size_t)CRYPTO_BYTES) {
      fprintf(stderr, "Signed message lengths wrong\n");
      exit(1);
    }
   	
		memcpy((void*)&aux, (void*)m, (size_t)LENGTH_FIELD_LEN);
   	if(aux != data_size){
   		fprintf(stderr, "Received wrong message length (%lu)\n", aux);
   		exit(1);
   	}
   	...
   	trials = 0U;
		...
		pointer = (size_t) LENGTH_FIELD_LEN;
		memcpy((void*)&version, (void*)(m + pointer), (size_t)TIMESTAMP_LEN);
