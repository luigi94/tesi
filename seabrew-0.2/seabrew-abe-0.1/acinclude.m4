...
AC_DEFUN([SEABREW_CHECK],
[

AC_MSG_CHECKING(for libseabrew)

AC_ARG_WITH(
  seabrew-include,
  AC_HELP_STRING(
    [--with-seabrew-include=DIR],
    [look for the header seabrew.h in DIR rather than the default search path]),
  [SEABREW_CFLAGS="-I$withval"], [SEABREW_CFLAGS=""])

AC_ARG_WITH(
  seabrew-lib,
  AC_HELP_STRING(
    [--with-seabrew-lib=DIR],
    [look for libseabrew.a in DIR rather than the default search path]),
  [
    case $withval in
      /* ) true;;
      *  ) AC_MSG_ERROR([

You must specify an absolute path for --with-seabrew-lib.
]) ;;
    esac
    SEABREW_LIBS="-L$withval -lseabrew"
  ], [SEABREW_LIBS="-lseabrew"])

BACKUP_CFLAGS=${CFLAGS}
BACKUP_LIBS=${LIBS}

CFLAGS="${CFLAGS} ${SEABREW_CFLAGS} ${BSWABE_CFLAGS} ${PBC_CFLAGS} ${GMP_CFLAGS} ${GLIB_CFLAGS}"
LIBS="${LIBS} ${SEABREW_LIBS} ${BSWABE_LIBS} ${PBC_LIBS} ${GMP_LIBS} ${GLIB_LIBS}"

AC_TRY_LINK(
  [#include <glib.h>
   #include <pbc.h>
   #include <seabrew.h>],
  [seabrew_bswabe_pub_t* p;
   seabrew_bswabe_pub_free(p);],
  [
    AC_MSG_RESULT(found)
    AC_SUBST(SEABREW_CFLAGS)
    AC_SUBST(SEABREW_LIBS)
    AC_DEFINE(HAVE_SEABREW,1,[Defined if libseabrew is installed])
  ],
  [
    AC_MSG_RESULT(not found)
    AC_MSG_ERROR([

The library libseabrew was not found on your system! Please obtain it
from

  http://acsc.csl.sri.com/cpabe/

and install it before trying again. If libseabrew is already
installed in a non-standard location, try again with

  ./configure --with-seabrew-include=<path> --with-seabrew-lib=<path>

If you already specified those arguments, double check that seabrew.h
can be found in the first path and libseabrew.a can be found in the
second.
