...
#if SECURITY == 128
#define PRIVATE_KEY_NAME_PREFIX "./../prime256v1_bits_keys/srvprvkey%u.pem"
#elif SECURITY == 192
#define PRIVATE_KEY_NAME_PREFIX "./../secp384r1_bits_keys/srvprvkey%u.pem"
#elif SECURITY == 256
#define PRIVATE_KEY_NAME_PREFIX "./../secp521r1_bits_keys/srvprvkey%u.pem"
#endif
...
void make_buffer_and_sign(const char* const restrict to_send_file_name, unsigned char* restrict* const restrict buffer, unsigned long* const total_len, char* const restrict prvkey_file_name){
  ...
  /* Adding cleartext */
	...
	*total_len += cleartext_len;
	...
	pointer = *total_len;
	exp_sgnt_size = (unsigned long) expected_key_size(prvkey_file_name) + 1U;
	*total_len += exp_sgnt_size;
	memcpy((void*)*buffer, (void*)&(*total_len), LENGTH_FIELD_LEN);
	
	sign(*buffer, *total_len - exp_sgnt_size, &sgnt_buf, exp_sgnt_size, prvkey_file_name);
	...
	memcpy((void*)(*buffer + pointer), (void*)sgnt_buf, exp_sgnt_size);
